...
const CustomPlaceOrderButton = (props: any) => {
  ...
  const { isActive, isTermsAccepted, ...rest } = props
...
export const StyledPlaceOrderButton = styled(
  ...
  ${({ isTermsAccepted }) =>
    isTermsAccepted
      ? null
      : tw`opacity-50 pointer-events-none cursor-not-allowed`}
`
...
export const PlaceOrderButtonWrapper = styled(ButtonWrapper)`
  ${tw`fixed flex-col w-full items-end bottom-[72px] md:bottom-9 p-5 bg-gray-50 z-20 border-t md:(static flex justify-end z-auto p-0 bg-white border-0)`}
  &::before {
